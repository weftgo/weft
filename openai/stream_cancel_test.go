...
// without Next() ever returning false, leaving stream.Err nil; before
// the ctx check after the read loop, that window turned a canceled run
// into a normal-looking finish (a canceled run with no tool calls then
// reported success).
